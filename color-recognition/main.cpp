...
{
    Mat img = imread("test3.jpg", IMREAD_COLOR);
    ...
    if(largest_triangle.size() <= 0)
    {
        cout << "no largest triangle!" << endl;
        exit(0);
    }
    
    // determine triangle color
    Rect cropped_rect = boundingRect(Mat(largest_triangle));
    ...
    for(int i = 0; i < largest_triangle.size(); i++)
    {
        line(img, largest_triangle[i % 3], largest_triangle[(i+1) % 3], Scalar(0, 255, 255), 4, 8);
    }
